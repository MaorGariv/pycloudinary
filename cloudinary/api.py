...
def ping(**options):
    return _call_api("get", ["ping"], {}, **options)


def usage(**options):
    return _call_api("get", ["usage"], {}, **options)


def resource_types(**options):
    return _call_api("get", ["resources"], {}, **options)
...
def resources(**options):
    ...
    params = _only(options, "next_cursor", "max_results", "prefix", "tags",
                  "context", "moderations", "direction", "start_at")
    return _call_api("get", uri, params, **options)


def resources_by_tag(tag, **options):
    resource_type = options.pop("resource_type", "image")
    uri = ["resources", resource_type, "tags", tag]
    params = _only(options, "next_cursor", "max_results", "tags",
...
def resources_by_ids(public_ids, **options):
    ...
    params = dict(_only(options, "tags", "moderations", "context"), public_ids=public_ids)
    return _call_api("get", uri, params, **options)
...
def resource(public_id, **options):
    ...
    params = _only(options, "exif", "faces", "colors", "image_metadata",
...
def update(public_id, **options):
    ...
    upload_type = options.pop("type", "upload")
    uri = ["resources", resource_type, upload_type, public_id]
    params = _only(options, "moderation_status", "raw_convert",
    ...
    return _call_api("post", uri, params, **options)
...
def delete_resources(public_ids, **options):
    ...
    return _call_api("delete", uri, params, **options)
...
def tags(**options):
    ...
    return _call_api("get", uri, _only(options, "next_cursor", "max_results", "prefix"), **options)


def transformations(**options):
    uri = ["transformations"]
    return _call_api("get", uri, _only(options, "named", "next_cursor", "max_results"), **options)


def transformation(transformation, **options):
    uri = ["transformations", _transformation_string(transformation)]
    return _call_api("get", uri, _only(options, "next_cursor", "max_results"), **options)


def delete_transformation(transformation, **options):
    uri = ["transformations", _transformation_string(transformation)]
    return _call_api("delete", uri, {}, **options)
...
def update_transformation(transformation, **options):
    uri = ["transformations", _transformation_string(transformation)]
    updates = _only(options, "allowed_for_strict")
    if "unsafe_update" in options:
        updates["unsafe_update"] = _transformation_string(options.get("unsafe_update"))
    if not updates:
        raise Exception("No updates given")
    return _call_api("put", uri, updates, **options)


def create_transformation(name, definition, **options):
    uri = ["transformations", name]
    return _call_api("post", uri, {"transformation": _transformation_string(definition)}, **options)


def publish_by_ids(public_ids, **options):
    resource_type = options.pop("resource_type", "image")
    uri = ["resources", resource_type, "publish_resources"]
    params = dict(_only(options, "type", "overwrite", "invalidate"), public_ids=public_ids)
    return _call_api("post", uri, params, **options)
...
def publish_by_prefix(prefix, **options):
    resource_type = options.pop("resource_type", "image")
    uri = ["resources", resource_type, "publish_resources"]
    params = dict(_only(options, "type", "overwrite", "invalidate"), prefix=prefix)
    return _call_api("post", uri, params, **options)
...
def publish_by_tag(tag, **options):
    resource_type = options.pop("resource_type", "image")
    uri = ["resources", resource_type, "publish_resources"]
    params = dict(_only(options, "type", "overwrite", "invalidate"), tag=tag)
    return _call_api("post", uri, params, **options)
...
def upload_preset(name, **options):
    ...
    return _call_api("get", uri, _only(options, "max_results"), **options)
...
def update_upload_preset(name, **options):
    ...
    params.update(_only(options, "unsigned", "disallow_public_id"))
    return _call_api("put", uri, params, **options)
...
def create_upload_preset(**options):
    ...
    params = utils.build_upload_params(**options)
    params = utils.cleanup_params(params)
    params.update(_only(options, "unsigned", "disallow_public_id", "name"))
    return _call_api("post", uri, params, **options)


def root_folders(**options):
    return _call_api("get", ["folders"], {}, **options)


def subfolders(of_folder_path, **options):
    return _call_api("get", ["folders", of_folder_path], {}, **options)
...
def update_upload_mapping(name, **options):
    uri = ["upload_mappings"]
    params = dict(folder=name)
    params.update(_only(options, "template"))
    return _call_api("put", uri, params, **options)
...
def list_streaming_profiles(**options):
    ...
    return _call_api('GET', uri, {}, **options)
...
def _call_json_api(method, uri, jsonBody, **options):
...
def _only(source, *keys):
    return {key: source[key] for key in keys if key in source}


def _transformation_string(transformation):
...
def __prepare_streaming_profile_params(**options):
    params = _only(options, "display_name")
    if "representations" in options:
        representations = [{"transformation": _transformation_string(trans)}
...
def __delete_resource_params(options, **params):
    p = dict(transformations=utils.build_eager(options.get('transformations')),
             **_only(options, "keep_original", "next_cursor", "invalidate"))
    p.update(params)
    return p
